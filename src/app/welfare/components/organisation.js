...
export default function Organisation({ title, desc, imageSrc, routeLink }) {
  ...
  return (
    <div>
      <div>
        <h1 className="text-center text-2xl text-cyan-700 font-semibold my-5"> Start Volunteering! </h1>
      </div>
      <div className="lg:w-11/12 w-8/12 md:w-full mx-auto grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4 overflow-auto max-h-[70vh]">
        {data.map((item, index) => (
          <div className="flex justify-center p-4" key={index}>
            <div className="max-w-sm bg-white border border-gray-200 rounded-lg shadow">
              <div className="p-5">
                ...
                <div className="md:h-[16vh] lg:h-[15vh]">
                  <p className="my-3 text-md font-normal text-gray-700 dark:text-gray-400">{item.desc}</p>
                </div>
                <a
                  href={item.routeLink}
